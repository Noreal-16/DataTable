class DataTable {
    ...
    constructor(selector, headerButton) {
    ...
    renderRows() {
        ...
        for (let i = pointer; i < limit; i++) {
            //console.log("Los datos ",this.copyItems[i]);
            if (i === total) break;
    ...
    renderPagesButtons() {
        ...
        const buttonToShow = this.pagination.noButtonsBeforeDots;
        ...
        const missingButton = buttonToShow - (limS - limI);
    ...
    renderHeaderButtons() {
        let html = '';
        const buttonContainer = this.element.querySelector('.header-buttons-container');
        ...
        headerButtons.forEach(button => {
            document.querySelector('#' + button.id).addEventListener('click', button.acction);
        });
    }
    renderSearch() {
